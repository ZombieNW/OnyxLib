...
OnyxLib is a framework for creating custom content using PaperMC. All custom content maps directly to real, pre-existing vanilla objects.
...
## 0. Download OnyxLib
Download from [GitHub](https://github.com/ZombieNW/OnyxLib/releases/latest) or [Modrinth](https://modrinth.com/plugin/onyxlib)
...
public class MyPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        ns.block("marble")
            ...
            .displayName("Marble")
            .blockDisplay(display -> display.all("blocks/marble"));
    }
}
```
...
OnyxLib copies assets from your plugin JAR's resource folder following standard Minecraft conventions to generate resource packs.
...
After creating your items or blocks, and your textures are in place, build your plugin `.jar` file, drop it in your server's `plugins/` folder **alongside** `OnyxLib.jar`, and start the server.
...
```text
[OnyxLib] Resource pack generated: resourcepack.zip (2 items)
```

You can find the ready-to-use zip archive located at `plugins/OnyxLib/OnyxLib-generated.zip`.

## 4. Get Your Items

To test your new item or block, use the built-in give command:

```text
/onyx give myplugin:ruby
/onyx give myplugin:marble 64
```
